...
const DEFAULT_SCENARIO_MODEL: GeminiModelOption = 'gemini-2.5-flash';
...
const SCENARIO_SEVERITIES = ['low', 'medium', 'high'] as const;
const SCENARIO_TIME_HORIZONS = ['immediate', 'six_months', 'one_to_two_years'] as const;
const SCENARIO_DIRECTIONS = ['decrease', 'no_material_change', 'increase'] as const;
const SCENARIO_ACTIONS = ['generate', 'evaluate'] as const;
...
const cleanTruncatedText = (value: unknown, maxLength: number): string => {
  ...
  if (sentenceWithinLimit?.[1]) return sentenceWithinLimit[1].trim();
  const truncated = cleaned.slice(0, maxLength);
  const lastSpace = truncated.lastIndexOf(' ');
  if (lastSpace > Math.floor(maxLength * 0.6)) return truncated.slice(0, lastSpace).trim();
...
const cleanWordLimitedText = (value: unknown, maxWords: number, maxLength: number): string => {
  ...
  if (words.length <= maxWords && cleaned.length <= maxLength) return cleaned;
...
const cleanSentenceLimitedText = (value: unknown, maxWords: number, maxLength: number): string => {
  ...
  if (chosen) return finalizeSentence(chosen);
...
const sanitizeScenarioInput = (value: unknown): ScenarioInput | null => {
  ...
  if (scenarioType === 'custom_scenario' && !customScenarioLabel) return null;
  return {
    scenarioType,
    customScenarioLabel,
    severity,
...
const sanitizeGeneratedScenario = (value: unknown, brandName: string): GeneratedScenario => {
...
const sanitizeUserPrediction = (value: unknown): UserPrediction | null => {
  ...
  if (!selectedManagementDecision) return null;
...
const sanitizeStrategyActions = (value: unknown): StrategyAction[] | null => {
...
const sanitizeBaselineAnalysis = (value: unknown): CBBEData | null => {
  if (!isRecord(value)) return null;
...
const parseScenarioGenerateResponse = (jsonText: string, brandName: string): GeneratedScenario => {
...
const parseScenarioEvaluateResponse = (jsonText: string, baselineAnalysis: CBBEData, strategyActions: StrategyAction[]): ScenarioEvaluation => {
  const parsed = parseJsonText(jsonText);
  ...
  const suggestedStrategyRevisions = cleanTextList(record.suggestedStrategyRevisions, 6, 220).map((text, index) => ({
    id: `revision-${index + 1}`,
    text,
    status: 'pending' as StrategyRevision['status']
  }));
  ...
  const actionCritiques = strategyActions.map((action, index) => {
    ...
    const critiqueRecord = isRecord(critiqueItems[index]) ? critiqueItems[index] : {};
    return {
      actionId: action.id,
      critique: cleanText(critiqueRecord.critique, 260) || 'No critique generated for this action.'
    };
  });

  return {
    scenarioSummary: cleanText(record.scenarioSummary, 700),
    strongReasoning: cleanTextList(record.strongReasoning, 6, 260),
    questionableAssumptions: cleanTextList(record.questionableAssumptions, 6, 260),
    missingConsiderations: cleanTextList(record.missingConsiderations, 6, 260),
    unintendedConsequences: cleanTextList(record.unintendedConsequences, 6, 260),
    dimensions,
    actionCritiques,
...
const buildScenarioGeneratePrompt = (brandName: string, baselineAnalysis: CBBEData, scenarioInput: ScenarioInput): string => {
...
const buildScenarioEvaluatePrompt = (
  brandName: string,

...
export const generateCBBEDashboard = async (brandName: string, model: GeminiModelOption = DEFAULT_MODEL): Promise<CBBEData> => {
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      tools: [{googleSearch: {}}],
      temperature: 0.1, // Lower temperature for more deterministic, grounded output
    }
  });

  // --- STRICT GROUNDING VERIFICATION ---
  // Create a map of verified URLs found by the search tool to eliminate 404s.
  const verifiedSources = new Map<string, string>(); // URL -> Title
  ...
  const data: CBBEData = JSON.parse(jsonText);

  // Sanitize and Verify references
  if (data.references && Array.isArray(data.references)) {
      ...
      if (verifiedSources.size > 0) {
        if (verifiedSources.has(finalUrl)) {
          // Exact match found
          isValid = true;
          // If JSON title is poor, upgrade it with metadata title
          if (!finalTitle || finalTitle === finalUrl) {
             finalTitle = verifiedSources.get(finalUrl) || finalTitle;
          }
        } else {
          // Fuzzy match try: Check if a verified source contains the reference URL or vice versa
          // This handles cases where the model trimmed a query param or protocol
          const bestMatchKey = Array.from(verifiedSources.keys()).find(vUrl => 
             vUrl.includes(finalUrl.replace(/^https?:\/\/(www\.)?/, '')) || 
             finalUrl.includes(vUrl)
          );

          if (bestMatchKey) {
            isValid = true;
            finalUrl = bestMatchKey; // Snap to the working URL
         ...
         // Fallback if no grounding metadata is returned (rare with googleSearch tool)
         // We perform a basic syntax check
         try {
      ...
      if (!isValid) return null;
        ...
        return {
          ...
          url: finalUrl,
          category: cleanCategory as ReferenceCategory,
